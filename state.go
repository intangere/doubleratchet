...
func (s *State) skipMessageKeys(key Key, until uint) ([]skippedKey, error) {
	...
	for uint(s.RecvCh.N) < until {
		...
		s.OrderedSkippedKeys = append(s.OrderedSkippedKeys, key)
		// Increment key count
		s.KeysCount++
...
// original status key truncation does not truncate public keys only message keys so the map would infinitely grow
// this implementation deletes the oldest skipped keys and keeps at most MaxKeep keys
func (s *State) truncateKeys() {
	truncate_count := len(s.OrderedSkippedKeys) - int(s.MaxKeep)
	if truncate_count > 0 {
		keys := s.OrderedSkippedKeys[0:truncate_count]
		s.OrderedSkippedKeys = s.OrderedSkippedKeys[truncate_count:]

		for _, key := range(keys) {
			s.MkSkipped.DeletePk(key)
		}
	}
}
