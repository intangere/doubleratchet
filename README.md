...
### Basic usage example
Simply append HE to the end of the session creation functions to use header encryption. 
```go
package main
...
func main() {
	...
	bob, err := doubleratchet.NewHE([]byte("bob-session-id"), sk, keyPair, nil)
	if err != nil {
		log.Fatal(err)
	}

	// Alice MUST be created with the shared secret and Bob's public key.
	alice, err := doubleratchet.NewWithRemoteKeyHE([]byte("alice-session-id"), sk, keyPair.PublicKey(), nil)
	if err != nil {
		log.Fatal(err)
	}
